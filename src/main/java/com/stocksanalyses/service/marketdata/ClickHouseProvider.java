...
@Service
public class ClickHouseProvider implements MarketDataProvider {
    ...
    public List<Candle> getCandles(String symbol, String interval, Instant start, Instant end, AdjustType adjustType) {
        ...
        try {
            ...
            List<Candle> rawCandles = clickHouseTemplate.query(sql, new CandleRowMapper(), 
    ...
    public Optional<Candle> getLatestCandle(String symbol, String interval) {
        ...
        try {
            ...
            List<Candle> candles = clickHouseTemplate.query(sql, new CandleRowMapper(), symbol, interval);
        ...
        try {
            String sql = String.format("""
                SELECT 
                    ...
                    count(*) as total,
                    count(*) * 1.0 / dateDiff('day', min(timestamp), max(timestamp)) as completeness
                FROM %s 
                WHERE symbol = ? AND interval = ?
                """, tableName);
            
            return clickHouseTemplate.queryForObject(sql, (rs, rowNum) -> {
    ...
    public DataQuality getDataQuality(String symbol, String interval, Instant start, Instant end) {
        ...
        if (clickHouseTemplate == null) {
            DataQuality fallback = new DataQuality(0.5, 0.5, 0.5, 0.5, 0, 0, 0, 
                List.of("ClickHouse not available"), Map.of());
            qualityCache.put(cacheKey, fallback);
            return fallback;
        }
        
        try {
            // ClickHouse-specific quality analysis
            String completenessSql = String.format("""
                SELECT 
                    count(*) as actual,
                    dateDiff('day', ?, ?) as expected_days
                FROM %s 
                WHERE symbol = ? AND interval = ? AND timestamp >= ? AND timestamp <= ?
                """, tableName);
            ...
            String consistencySql = String.format("""
                SELECT 
                    count(*) as duplicates,
                    countIf(open > high OR open < low OR close > high OR close < low) as invalid
                FROM %s 
                WHERE symbol = ? AND interval = ? AND timestamp >= ? AND timestamp <= ?
                """, tableName);
            
            // Calculate quality metrics
            var completenessResult = clickHouseTemplate.queryForMap(completenessSql, start, end, symbol, interval, start, end);
            var consistencyResult = clickHouseTemplate.queryForMap(consistencySql, symbol, interval, start, end);
            
            long actual = ((Number) completenessResult.get("actual")).longValue();
            long expectedDays = ((Number) completenessResult.get("expected_days")).longValue();
            ...
            double completeness = expectedDays > 0 ? Math.min(1.0, actual / (double) expectedDays) : 0.0;
            ...
            DataQuality quality = new DataQuality(overallScore, completeness, consistency, accuracy,
                expectedDays - actual, duplicates, invalid, issues, Map.of(
    ...
    public ProviderHealth getHealth() {
        if (clickHouseTemplate == null) {
            return new ProviderHealth(false, "ClickHouse not configured", Instant.now(), 0, 
                Map.of("error", "ClickHouseTemplate not available"));
        }
        
        long startTime = System.currentTimeMillis();
        try {
            clickHouseTemplate.queryForObject("SELECT 1", Integer.class);
            ...
            long responseTime = System.currentTimeMillis() - startTime;
            return new ProviderHealth(false, "ClickHouse error", Instant.now(), responseTime, 
    ...
    private List<Candle> getFallbackCandles(String symbol, String interval, Instant start, Instant end) {
        // Fallback to stub data when ClickHouse is not available
